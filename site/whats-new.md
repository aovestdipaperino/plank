...
that actually differs for Qwen is read from the model's own architecture, so
`-m some-qwen.gguf` behaves the same way.

**Qwen speaks a different tool-call dialect, and plank now understands it.** It
does not write DSML; it writes `<tool_call>` / `<function=…>` /
`<parameter=…>` and expects JSON schemas inside `<tools>`. Before this every
tool call it made came back as `invalid DSML tool call` — one recorded session
tried four times and then told the user the harness was broken, which it was.

**Speculative decoding has one name: `--mtp`.** Multi-token prediction is one
idea with two mechanisms — DeepSeek speculates from its DSpark draft
checkpoint, Qwen from the MTP block inside its own main GGUF — so it is one
flag and one switch now. **This is a breaking change**: `/dspark` is `/mtp`,
`--dspark*` are `--mtp*`, and both the old `--mtp PATH` and `--ple PATH` are
