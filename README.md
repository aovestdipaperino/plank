...
- **Expanded hooks** — beyond `PreToolUse`/`PostToolUse`/`Stop`, hooks now fire on `UserPromptSubmit`, `SessionStart`, `SessionEnd`, `PreCompact`/`PostCompact`, and `PostToolUseFailure`; several can inject context into the turn. An optional JSON response envelope adds `continue:false`, non-blocking `systemMessage`, `suppressOutput`, and `async` execution over the existing exit-code protocol; matchers can key on arguments (`bash(git *)`, `write(*.md)`); and a `prompt` hook type feeds text to the model instead of running a process.
- **Directory-aware history** — prompt and `!` history is tagged with the directory it was entered in, so scrolling history in one project no longer surfaces another project's commands; legacy untagged history stays universally visible. Composes with the mode-aware `!` filter (mode × directory).
