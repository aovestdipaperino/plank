...
<p align="center">
  ...
  <a href="https://plank-agent.dev"><img src="https://img.shields.io/badge/website-plank--agent.dev-blue" alt="Website: plank-agent.dev"></a>
...
- `--mtp-confidence F` — pruning threshold, `0..1`. `0` forces fixed five-token blocks (diagnostics). The default is the engine's own and depends on the backend.
- `--mtp-strict` — load the drafter but keep target-only decode, for comparisons and correctness checks.
- `/mtp on|off` — the same switch mid-session, between turns.
...
| `safety` | `sandbox` | on (macOS) | Default for the write sandbox: bash runs under Seatbelt, and `write`/`edit` refuse paths outside the workspace. Same as `--sandbox`/`--no-sandbox`. |
