...
- **`/btw`** — ask a quick side question over the shared conversation context without touching it: one tool-free response, nothing enters the transcript, and the KV cache rolls back automatically on the next real turn. In the TUI the prompt stays live for `/btw` even while the agent is working: a question typed mid-turn is queued and answered at the next generation boundary.
