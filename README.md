...
- **Remote control** — a running instance can open a loopback WebSocket so another process, a browser, or the `plank remote <url>` terminal client can attach to it: it mirrors the output, sends prompts/commands/`/btw`/interrupts, and takes or hands back control (single controller, many mirrors, with a reconnect grace window). A self-contained web client is served at `/`. Auth is a bearer token, generated and printed once when the server starts. Inside a running TUI session, `/remote-control` (alias `/rc`) is the only way to start it: bare `/rc` toggles, `/rc on`/`/rc off` are explicit and case-insensitive. Turning it on binds an ephemeral loopback port, mints a fresh token, and prints `http://127.0.0.1:PORT/?t=TOKEN` plus an `ssh -L` tunnel hint; opening that link auto-connects the bundled web client and claims control immediately, no token to paste and no button to press (the token still lands in browser history and any `Referer`, so treat the link as a convenience for a loopback-only listener, not a secret). The link is the only way in: the token is printed nowhere else, so the page carries no URL or token fields to fill and a page opened without `?t=` says so rather than offering controls that cannot work. The bundled page currently sends prompts, commands and `/btw` only — its interrupt and take-control buttons are commented out, since it claims control on connect; the `plank remote <url>` client still does both. `/rc off` tells connected clients, shuts the listener down, and kills the token with it, so a stale link is refused; a later `/rc` mints a new port and token. Remote control requires the full-screen TUI: `/rc` in a piped or headless session declines rather than starting a server nothing can drive.
