...
- **KV-cache discipline** (`docs/KV-CACHE.md` for the full mechanics): `Ds4Engine` keeps one live session across turns so only the new suffix is prefilled; the system prompt has a fingerprinted disk snapshot (`sysprompt.kv`). Reuse only genuinely matching token prefixes — a stale checkpoint is rebuilt, never trusted.
