...
- **KV-cache discipline** (`docs/KV-CACHE.md` for the full mechanics): `Ds4Engine` keeps one live session across turns so only the new suffix is prefilled; the system prompt has a fingerprinted disk snapshot (`sysprompt-<fp1>.kv_raw`). Reuse only genuinely matching token prefixes: a blob's embedded signature is the sole trust input, its sidecar is advisory, and a stale checkpoint is rebuilt rather than trusted.
