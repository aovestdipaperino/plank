...
//! Pieces the games share, ported from plank's `src/arcade.rs`.
//!
//! Verbatim, for the same reason the faces are: a game's physics is tuned
//! against these exact numbers, and a port that quietly rounded one would play
//! differently in a way only a glyph-for-glyph comparison could find.
