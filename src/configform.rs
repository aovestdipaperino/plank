...
mod tests {
    ...
    #[test]
    fn ui_cursor_is_not_a_config_key() {
        // The drawn cursor is not optional: there is no OSC path left to fall
        // back to, so there is nothing for a setting to select.
        let mut s = Settings::default();
        let err = set_from_path(&mut s, "ui.cursor", "drawn").unwrap_err();
        assert_eq!(err, "unknown setting: ui.cursor");
    }
