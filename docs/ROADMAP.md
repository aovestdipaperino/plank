...
- ~~**Remote-control interface**~~ ([#25](https://github.com/aovestdipaperino/plank/issues/25)) — drive a running plank instance from another process or machine. Design: [`REMOTE-CONTROL-DESIGN.md`](REMOTE-CONTROL-DESIGN.md). Transport foundation in `c7c37a7` (WebSocket protocol, `BroadcastBus` scrollback replay, token auth, single-controller/many-mirror policy, launch-time `--control*` flags); **live wiring landed** in `243ba8a` — the server shares the agent's `TurnShared`/bus, so remote `prompt`/`command`/`btw`/`interrupt` frames drive the real turn loop and output mirrors, with a 15s reconnect grace window. **Landed** on the `rc-toggle` branch: the `--control*` launch flags were replaced by an in-session `/remote-control` (`/rc`) toggle that prints a tokenized one-click link. The `plank remote <url>` CLI client landed earlier, on `main` in `ddb0459`, and still works against a server started with `/rc`. The plain-REPL (non-TTY) and headless remote-drive paths were deleted rather than finished, since starting the server now requires typing a slash command in the full-screen TUI — remote control is TUI-only.
